#!/usr/bin/env python3
"""
bearimetric_fleet.py

Bulk ADB operations for Bearimetric scouting tablets.
...
Usage:
    python bearimetric_fleet.py          (requires adb + flutter in PATH)
...
PACKAGE_ID  = "org.tahomarobotics.bearimetric"
...
def op_export(serial: str) -> None:
    ...
    zip_name  = f"bearimetric_export_{safe_ser}_{timestamp}.zip"
...
@operation(
    "reset",
    "Reset App Data",
    "Clear ALL Bearimetric app data on the device (cache, prefs, Hive boxes — irreversible)",
)
...
def _configure_provision() -> bool:
    ...
    print()
    print("  Paste the Bearimetric provisioning payload (from Beariscope).")
...
def show_menu() -> list[Operation]:
    ...
    while True:
        print("\n" + "─" * 50)
        print("  Bearimetric Fleet Manager")
        print("─" * 50)
        print("  Operations:")
